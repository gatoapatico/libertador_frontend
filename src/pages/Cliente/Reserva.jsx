...
export default function Reserva() {
  ...
  const checkOutDate = `${endDate.getFullYear()}-${
    endDate.getMonth() + 1
  }-${endDate.getDate()}`;
  ...
  useEffect(() => {
    async function getCategoriasByHabitaciones() {
      const res = await fetch(
        `http://localhost:8080/api/habitaciones/disponibles?fechaCheckIn=${checkInDate}&fechaCheckOut=${checkOutDate}`
      );
      ...
      data.forEach((habitacion) => {
        if (habitacion.disponibilidad === "Disponible") {
          dataIndexCategoriasDisponibles = {
            ...dataIndexCategoriasDisponibles,
            [habitacion.tipoHabitacion.id]: !dataIndexCategoriasDisponibles[
              habitacion.tipoHabitacion.id
            ]
              ? 1
              : dataIndexCategoriasDisponibles[habitacion.tipoHabitacion.id] +
                1,
          };
        }
      });
    ...
    async function seteoDeCategorias(dataIndexCategoriasDisponibles) {
      ...
      const categoriasDisponibles = data.map((categoria) => {
        if (
          Object.keys(dataIndexCategoriasDisponibles)
            .map((id) => parseInt(id))
          ...
          return {
            ...
            habitacionesDisponibles:
              dataIndexCategoriasDisponibles[categoria.id],
          };
        } else {
          return {
            ...categoria,
      ...
      <div key={nanoid()} className={styleHabitacion}>
        ...
        <div className="habitacion-info">
          ...
          <button
            ...
            onClick={() => handlePopDetalle(categoria)}
          >
            Detalles de la habitación
          </button>
          <div className="reserva-info">
            <div className="info-adicional">
              <button className="btn-terminos" onClick={handleTermCond}>
                Términos y condiciones
              </button>
              <h4>
                <i className="bi bi-credit-card-fill"></i>Depósito obligatorio
              </h4>
            </div>
            <div className="precio-reserva">
              ...
              <button
                disabled={isDisabled}
                className={btnStyles}
                onClick={() => handleReserva(categoria)}
              >
                {txtBtnReservar}
              </button>
            </div>
  ...
  return (
    <div className="contenedor-reservas">
      <div className="main-reservas">
        <main className="reservas-container">
          <div className="reservas-user">
            ...
            <div className="habitaciones">{categoriasEl}</div>
          </div>
          {categoriaSelected != null ? (
            <div className="reservas-resumen">
              ...
              <p>
                ...
                <span>{categoriaSelected.precioCategoria.toFixed(2)}</span>
              </p>
              <p>
                Servicios
                <span>{categoriaSelected.costoServicios.toFixed(2)}</span>
              </p>
              <p>
                IGV(18%)
                <span>
                  {(categoriaSelected.costoTotalCategoria * 0.18).toFixed(2)}
                </span>
              </p>
              <div className="resumen-total">
                <p>
                  ...
                  <span>
                    S/
                    {(
                      categoriaSelected.costoTotalCategoria +
                      categoriaSelected.costoTotalCategoria * 0.18
                    ).toFixed(2)}
                  </span>
                </p>
              </div>
              <button className="btn-pagar-reserva" onClick={handlePagoReserva}>
                PAGAR RESERVA
              </button>
            </div>
          ) : (
      ...
      {isTermCond ? (
        <TerminosCondiciones handleTermCond={handleTermCond} />
      ) : (
        ""
      )}
      {isPopDetalle ? (
        <DetalleHabitacion
          handlePopDetalle={handlePopDetalle}
          categoriaPop={categoriaPop}
        />
      ) : (
        ""
      )}
